export const TELEMETRY_SCHEMA_VERSION = "veilance.telemetry.v0.1";
...
export function createEmptyState(tabId, url, now = Date.now()) {
...
export function addNetworkRequest(state, details, now = Date.now()) {
  ...
  const tracker = classifyTrackerHost(host);
  if (tracker) {
...
export function buildSanitizedPayload(state, extensionVersion = "0.0.0", now = Date.now()) {
  ...
  return {
    ...
    site: {
      ...
      observedUntil: new Date(state.updatedAt).toISOString()
    },
    summary,
    network: {
