...
function tarSize(bytes, offset) {
  ...
  if (!value) return 0;
  if (!/^[0-7]+$/.test(value)) throw new Error("Veilance database archive contains an invalid TAR size field");
  const size = Number.parseInt(value, 8);
  if (!Number.isSafeInteger(size) || size < 0) throw new Error("Veilance database archive contains an unsafe TAR entry size");
  return size;
}
...
export function extractTrackerDocumentsFromTar(buffer, folderName = "veilance-json-trackers") {
  ...
  while (offset + TAR_BLOCK_BYTES <= bytes.length) {
    ...
    if (dataEnd > bytes.length) throw new Error("Veilance database archive ended inside a TAR entry");
    ...
    if (type === "x") {
      ...
      if (sourceName) {
        if (size > MAX_TRACKER_FILE_BYTES) {
          throw new Error(`${sourceName} is larger than the 256 KB JSON record limit`);
        }
        relevantBytes += size;
        if (relevantBytes > MAX_TRACKER_TOTAL_BYTES) {
          throw new Error("Veilance database JSON exceeds the 24 MB safety limit");
        }
        if (documents.length >= MAX_TRACKER_FILES) {
          throw new Error(`Veilance database archive contains more than ${MAX_TRACKER_FILES} JSON files`);
        }
  ...
  if (!documents.length) {
    throw new Error(`Veilance database archive does not contain ${folderName}/*.json records`);
  }
  return documents;
}
...
async function readStreamWithLimit(stream, maximumBytes) {
  ...
  while (true) {
    ...
    if (total > maximumBytes) {
      ...
      throw new Error("Expanded Veilance database archive exceeds the 64 MB safety limit");
    }
    chunks.push(value);
  }
...
export async function fetchJsonDatabaseArchive(
  url,
  folderName = "veilance-json-trackers",
  fetchImpl = fetch
) {
  const response = await fetchImpl(url, { cache: "no-store", redirect: "follow" });
  if (!response.ok) throw new Error(`Veilance database repository returned HTTP ${response.status}`);
  const declaredSize = Number(response.headers.get("content-length") || 0);
  if (declaredSize > MAX_COMPRESSED_ARCHIVE_BYTES) {
    throw new Error("Compressed Veilance database archive exceeds the 8 MB safety limit");
  }
  const compressed = new Uint8Array(await response.arrayBuffer());
  if (!compressed.length || compressed.byteLength > MAX_COMPRESSED_ARCHIVE_BYTES) {
    throw new Error("Downloaded Veilance database archive is empty or exceeds the 8 MB safety limit");
  }
  if (typeof DecompressionStream !== "function") {
    throw new Error("This browser does not support secure gzip decompression for database updates");
  }
  ...
  return {
    ...
    documents: extractTrackerDocumentsFromTar(expanded, folderName)
  };
}

export function fetchTrackerArchive(url, fetchImpl = fetch) {
  return fetchJsonDatabaseArchive(url, "veilance-json-trackers", fetchImpl);
}
