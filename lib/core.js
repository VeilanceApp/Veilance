...
export function addProtectionEvent(state, event, now = Date.now()) {
  ...
  const clean = {
    surface: String(event.surface || "Protected surface").slice(0, 80),
    action: String(event.action || "Protected").slice(0, 80),
    technique: String(event.technique || "Fingerprint protection").slice(0, 120),
    beforeSignature: String(event.beforeSignature || "").slice(0, 32),
    afterSignature: String(event.afterSignature || "").slice(0, 32),
    changedUnits: Math.max(0, Math.min(1000000, Number(event.changedUnits) || 0)),
    explanation: String(event.explanation || "").slice(0, 300),
    timestamp: Number.isFinite(event.timestamp) ? event.timestamp : now
  };
  state.protections.total = Math.max(0, Number(state.protections.total) || 0) + 1;
  state.protections.lastProtectedAt = clean.timestamp;
  state.protections.events.unshift(clean);
  if (state.protections.events.length > 50) state.protections.events.length = 50;
  state.updatedAt = Math.max(state.updatedAt || 0, now);
  return state;
}
